import {
  ...
  //internal import
  import ApplicantsTable from "@/components/applicant/ApplicantsTable";
...
import MultipleDeleteModal from "@/components/modal/MultipleDeleteModal";
  ...
  const [data, setData] = useState({ applicants: [], totalApplicants: 0 });
  ...
  const   fetchData = async (params) => {
    setLoading(true);
    try {
      const result = await EmployeeServices.getAdminDashboardRecentApplicants(params);
      setData(result);
      setError(null);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const {
      ...
      dateRangeType,
      handleSubmitApplicants,
      handleDateRangeTypeChange,
      handleSortChange,
      setDateRange,
    } = useFilter(data?.applicants);
    ...
    const handleSelectAll = () => {
      if (selectedApplicants.length === dataTable.length) {
        setSelectedApplicants([]);
      } else {
        setSelectedApplicants(dataTable.map(applicant => applicant._id));
      }
    };
    ...
    const handleDeleteSelected = async () => {
      ...
      const selectedApplicantsData = data.applicants.filter(applicant => 
          selectedApplicants.includes(applicant._id)
      );
      
      // If no companies are found, show an error and return
      if (selectedApplicantsData.length === 0) {
        notifyError("Selected applicants not found in the data");
        return;
      }

      // Create a string of company names
      const applicantNames = selectedApplicantsData.map(applicant => applicant.name).join(", ");
      
      console.log('applicantNames', title);

      handleMultipleDeleteModalOpen(selectedApplicants, applicantNames);
    };
  ...
  const handleDownloadRequest = async (selectedColumnKeys, onlySelected) => {
    try {
      ...
      notifySuccess("Applicants download initiated successfully");
    } catch (error) {
  ...
  const sortOptions = [
    ...
    { value: "appliedAt", label: "Applied On" },
    { value: "age", label: "Age" },
...
const resultsPerPageOptions = [
  ...
  { value: 160, label: '160' },
    ...
    return (
      <>
        <PageTitle>{t("ApplicantsPage")}</PageTitle>
  ...
  <Card className="min-w-0 shadow-xs overflow-visible bg-white dark:bg-gray-800 mb-5">
    <CardBody>
      <form
        onSubmit={handleSubmitApplicants}
      ...
      >
        ...
        <div className="col-span-1 md:col-span-2 lg:col-span-3 xl:col-span-4 relative">
          <Input
            ...
            type="search"
            name="search"
            placeholder={t("ApplicantsPageSearchPlaceholder")}
          />
          <button
            type="submit"
...
<div className=" w-20 justify-self-end">
           {selectedApplicants.length > 0 && (
            ...
            >
              <IoTrashOutline className="mr-2 size-4"/> ({selectedApplicants.length})
            </Button>
          )}
          </div>
        </div>
          ...
          <TableContainer className="mb-8">
            ...
            <div className="my-2 p-2">
              ...
              <Button 
                  ...
                  className="ml-2 text-white dark:text-white font-bold bg-sky-900 hover:bg-sky-700 active:bg-sky-800"
                >
                  Clear
                </Button>
            </div>
  ...
  <Pagination
    totalResults={data?.totalApplicants}
    ...
    >
      {resultsPerPageOptions.map((option) => (
        <option key={option.value} value={option.value}>
                ...
                <tr className="bg-gray-700 text-white dark:bg-gray-600 dark:text-white">
                  <TableCell className="text-center">
                    <input
                      ...
                      checked={selectedApplicants.length > 0 && selectedApplicants.length === dataTable.length}
                      onChange={handleSelectAll}
                    />
                  </TableCell>
                  <TableCell className="text-center" onClick={() => handleSortByField("userId")}>{t("CandidateId")} {sortBy === "userId" && (
                    <span className="ml-2 text-white">{sortOrder === "asc" ? "▲" : "▼"}</span>
                  )}</TableCell>
                  <TableCell className="text-center" onClick={() => handleSortByField("appliedAt")}>{t("AppliedOn")} {sortBy === "appliedAt" && (
                    <span className="ml-2 text-white">{sortOrder === "asc" ? "▲" : "▼"}</span>
                  )}</TableCell>
                  <TableCell className="text-center" onClick={() => handleSortByField("fullName")}>{t("CandidateName")} {sortBy === "fullName" && (
                    <span className="ml-2 text-white">{sortOrder === "asc" ? "▲" : "▼"}</span>
                  )}</TableCell>
                  <TableCell className="text-center" onClick={() => handleSortByField("mobile")}>{t("CandidateMobile")} {sortBy === "mobile" && (
                    <span className="ml-2 text-white">{sortOrder === "asc" ? "▲" : "▼"}</span>
                  )}</TableCell>
                  <TableCell className="text-center" onClick={() => handleSortByField("companyName")}>{t("CompanyName")} {sortBy === "companyName" && (
                    <span className="ml-2 text-white">{sortOrder === "asc" ? "▲" : "▼"}</span>
                  )}</TableCell>
                  <TableCell className="text-center" onClick={() => handleSortByField("jobTitle")}>{t("JobTitle")} {sortBy === "jobTitle" && (
                    <span className="ml-2 text-white">{sortOrder === "asc" ? "▲" : "▼"}</span>
                  )}</TableCell>
                  <TableCell className="text-center" onClick={() => handleSortByField("status")}>{t("Status")} {sortBy === "status" && (
                    <span className="ml-2 text-white">{sortOrder === "asc" ? "▲" : "▼"}</span>
                  )}</TableCell>
                  <TableCell className="text-center">
                    {t("Actions")}
          ...
          <NotFound title="Sorry, There are no applicants right now." />
        )}
      </>
    );
  };
