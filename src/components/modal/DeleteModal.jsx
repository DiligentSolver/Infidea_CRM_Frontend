...
const DeleteModal = ({ id, ids, category, title, useParamId }) => {
  const { isModalOpen, closeModal, setIsUpdate, isMultipleDeleteModalOpen, closeMultipleDeleteModal } = useContext(SidebarContext);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (location.pathname === "/users") {
        if (ids) {
          const res = await EmployeeServices.deleteUsers({
            userIds: ids,
          });
          setIsUpdate(true);
          notifySuccess(res.message);
          ...
          const res = await EmployeeServices.deleteUser(id);
          setIsUpdate(true);
          notifySuccess(res.message);
          setServiceId();
          closeModal();
      ...
      if (location.pathname === "/jobs") {
        if(ids){
          const res = await EmployeeServices.deleteJobs({
            jobIds: ids,
          });
          setIsUpdate(true);
          notifySuccess(res.message);
          ...
          const res = await EmployeeServices.deleteJob(id);
          setIsUpdate(true);
          notifySuccess(res.message);
          setServiceId();
          closeModal();
      ...
      if (location.pathname === "/companies") {
          ...
          const res = await EmployeeServices.deleteCompany(id);
          setIsUpdate(true);
          notifySuccess(res.message);
          setServiceId();
          closeModal();
      ...
      if (location.pathname === "/our-staff") {
        const res = await EmployeeServices.deleteStaff(id);
        setIsUpdate(true);
        notifySuccess(res.message);
        setServiceId();
        closeModal();
